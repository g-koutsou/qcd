...
int main(int argc,char* argv[])
{
   ...
   FILE *fp_corrloc_s = NULL;      
   FILE *fp_corrloc_p = NULL;      
   ...
   FILE *fp_corrnoe_xv = NULL;                      // output files
   FILE *fp_corrloc_xs = NULL;      
   FILE *fp_corrloc_xp = NULL;      
   FILE *fp_corrloc_xv = NULL;      
   FILE *fp_corrloc_xa = NULL;      
   FILE *fp_corr_xvD = NULL;
   FILE *fp_corr_xaD = NULL;
   FILE *fp_corr_xd1 = NULL;
   ...
   char corrloc_xs_name[qcd_MAX_STRING_LENGTH];  // name of output file name scalar current moment
   char corrloc_xp_name[qcd_MAX_STRING_LENGTH];  // name of output file name pseudo-scalar current moment
   char corrloc_xv_name[qcd_MAX_STRING_LENGTH];  // name of output file name local vector current moment
   char corrnoe_xv_name[qcd_MAX_STRING_LENGTH];  // name of output file name noether vector current moment
   char corrloc_xa_name[qcd_MAX_STRING_LENGTH];  // name of output file name local axial current moment
   char corr_xvD_name[qcd_MAX_STRING_LENGTH];    // name of output file name one derivative vector moment
   char corr_xaD_name[qcd_MAX_STRING_LENGTH];    // name of output file name one derivative axial moment
   char corr_xd1_name[qcd_MAX_STRING_LENGTH];    // name of output file name one derivative axial antisymmetric moment
   ...
   strcpy(corrloc_xp_name,qcd_getParam("<corr_name_xp_local>",params,params_len));
   if(myid==0) printf("Got output file name: %s\n",corrloc_xp_name);
   
   strcpy(corrloc_xs_name,qcd_getParam("<corr_name_xs_local>",params,params_len));
   if(myid==0) printf("Got output file name: %s\n",corrloc_xs_name);
   
   strcpy(corrloc_xv_name,qcd_getParam("<corr_name_xv_local>",params,params_len));
   if(myid==0) printf("Got output file name: %s\n",corrloc_xv_name);
   
   strcpy(corrnoe_xv_name,qcd_getParam("<corr_name_xv_noether>",params,params_len));
   if(myid==0) printf("Got output file name: %s\n",corrnoe_xv_name);
   
   strcpy(corrloc_xa_name,qcd_getParam("<corr_name_xa_local>",params,params_len));
   if(myid==0) printf("Got output file name: %s\n",corrloc_xa_name);
     
   strcpy(corr_xvD_name,qcd_getParam("<corr_name_xvD>",params,params_len));
   if(myid==0) printf("Got output file name: %s\n",corr_xvD_name);
      
   strcpy(corr_xaD_name,qcd_getParam("<corr_name_xaD>",params,params_len));
   if(myid==0) printf("Got output file name: %s\n",corr_xaD_name);
         
   strcpy(corr_xd1_name,qcd_getParam("<corr_name_xd1>",params,params_len));
   if(myid==0) printf("Got output file name: %s\n",corr_xd1_name);
   ...
   qcd_complex_16 *corr[16], *corr_master[16];
...
#pragma omp parallel for private(id1,id2,id3,ip1,im1,ic1,ic2,ic3,i,mu,nu)
      ...
      if(t==t_start)         
      {
         ...
         if(myid==0)
         {
            ...
            printf("opening %s\n",corrloc_xs_name); fflush(stdout);
            fp_corrloc_xs = fopen(corrloc_xs_name,"w");   
            if(fp_corrloc_xs==NULL)
            {
               printf("failed to open %s for writing\n",corrloc_xs_name); fflush(stdout);
               k=1;
            }
            printf("opening %s\n",corrloc_xp_name); fflush(stdout);
            fp_corrloc_xp = fopen(corrloc_xp_name,"w");   
            if(fp_corrloc_xp==NULL)
            {
               printf("failed to open %s for writing\n",corrloc_xp_name); fflush(stdout);
               k=1;
            }
            printf("opening %s\n",corrnoe_xv_name); fflush(stdout);
            fp_corrnoe_xv = fopen(corrnoe_xv_name,"w");   
            if(fp_corrnoe_xv==NULL)
            {
               printf("failed to open %s for writing\n",corrnoe_xv_name); fflush(stdout);
               k=1;
            }
            printf("%s opened\n",corrnoe_xv_name); fflush(stdout);
            printf("opening %s\n",corrloc_xv_name); fflush(stdout);
            fp_corrloc_xv = fopen(corrloc_xv_name,"w"); fflush(stdout);
            if(fp_corrloc_xv==NULL)
            {
               printf("failed to open %s for writing\n",corrloc_xv_name); fflush(stdout);
               k=1;
            }            
            printf("%s opened\n",corrloc_xv_name); fflush(stdout);
            printf("opening %s\n",corrloc_xa_name); fflush(stdout);
            fp_corrloc_xa = fopen(corrloc_xa_name,"w");
            if(fp_corrloc_xa==NULL)
            {
               printf("failed to open %s for writing\n",corrloc_xa_name); fflush(stdout);
               k=1;
            }   
            printf("%s opened\n",corrloc_xa_name); fflush(stdout);
            printf("opening %s\n",corr_xvD_name); fflush(stdout);
            fp_corr_xvD = fopen(corr_xvD_name,"w");   
            if(fp_corr_xvD==NULL)
            {
               printf("failed to open %s for writing\n",corr_xvD_name); fflush(stdout);
               k=1;
            ...
            printf("%s opened\n",corr_xvD_name); fflush(stdout);
            printf("opening %s\n",corr_xaD_name); fflush(stdout);
            fp_corr_xaD = fopen(corr_xaD_name,"w");
            if(fp_corr_xaD==NULL)
            {
               printf("failed to open %s for writing\n",corr_xaD_name); fflush(stdout);
               k=1;
            ...
            printf("%s opened\n",corr_xaD_name); fflush(stdout);
            printf("opening %s\n",corr_xd1_name); fflush(stdout);
            fp_corr_xd1 = fopen(corr_xd1_name,"w");
            if(fp_corr_xd1==NULL)
            {
               printf("failed to open %s for writing\n",corr_xd1_name); fflush(stdout);
               k=1;
...
#pragma omp parallel for private(lx,ly,lz,v,x,y,z,tmp)
	...
	for(int j=0; j<numOfMom; j++)
      ...
      corr[0][0] = (qcd_complex_16) {0,0};
      corr[1][0] = (qcd_complex_16) {0,0};
      corr[2][0] = (qcd_complex_16) {0,0};
      corr[3][0] = (qcd_complex_16) {0,0};
      corr[4][0] = (qcd_complex_16) {0,0};
      corr[5][0] = (qcd_complex_16) {0,0};
      if(lt>=0 && lt<geo.lL[0])  //inside the local lattice, otherwise nothing to calculate
	for(lx=0; lx<geo.lL[1]; lx++)
	  for(ly=0; ly<geo.lL[2]; ly++)
	    for(lz=0; lz<geo.lL[3]; lz++)
		...
		y=ly+geo.Pos[2]*geo.lL[2] - x_src[2];
		z=lz+geo.Pos[3]*geo.lL[3] - x_src[3];
		corr[0][0]=qcd_CADD(corr[0][0], qcd_CSCALE(block_s[v], x));
		corr[1][0]=qcd_CADD(corr[1][0], qcd_CSCALE(block_s[v], y));
		corr[2][0]=qcd_CADD(corr[2][0], qcd_CSCALE(block_s[v], z));
		corr[3][0]=qcd_CADD(corr[3][0], qcd_CSCALE(block_p[v], x));
		corr[4][0]=qcd_CADD(corr[4][0], qcd_CSCALE(block_p[v], y));
		corr[5][0]=qcd_CADD(corr[5][0], qcd_CSCALE(block_p[v], z));
	      }   
      MPI_Reduce(&(corr[0][0].re), &(corr_master[0][0].re), 2, MPI_DOUBLE, MPI_SUM, 0, MPI_COMM_WORLD);
      MPI_Reduce(&(corr[1][0].re), &(corr_master[1][0].re), 2, MPI_DOUBLE, MPI_SUM, 0, MPI_COMM_WORLD);
      MPI_Reduce(&(corr[2][0].re), &(corr_master[2][0].re), 2, MPI_DOUBLE, MPI_SUM, 0, MPI_COMM_WORLD);
      MPI_Reduce(&(corr[3][0].re), &(corr_master[3][0].re), 2, MPI_DOUBLE, MPI_SUM, 0, MPI_COMM_WORLD);
      MPI_Reduce(&(corr[4][0].re), &(corr_master[4][0].re), 2, MPI_DOUBLE, MPI_SUM, 0, MPI_COMM_WORLD);
      MPI_Reduce(&(corr[5][0].re), &(corr_master[5][0].re), 2, MPI_DOUBLE, MPI_SUM, 0, MPI_COMM_WORLD);
      if(myid==0) 
	{
	  fprintf(fp_corrloc_xs,"%i %+i %+i %+i %+e %+e x\n",t,0,0,0,sign*corr_master[0][0].re,sign*corr_master[0][0].im);
	  fprintf(fp_corrloc_xs,"%i %+i %+i %+i %+e %+e y\n",t,0,0,0,sign*corr_master[1][0].re,sign*corr_master[1][0].im);
	  fprintf(fp_corrloc_xs,"%i %+i %+i %+i %+e %+e z\n",t,0,0,0,sign*corr_master[2][0].re,sign*corr_master[2][0].im);
	  fprintf(fp_corrloc_xp,"%i %+i %+i %+i %+e %+e x\n",t,0,0,0,sign*corr_master[3][0].re,sign*corr_master[3][0].im);
	  fprintf(fp_corrloc_xp,"%i %+i %+i %+i %+e %+e y\n",t,0,0,0,sign*corr_master[4][0].re,sign*corr_master[4][0].im);
	  fprintf(fp_corrloc_xp,"%i %+i %+i %+i %+e %+e z\n",t,0,0,0,sign*corr_master[5][0].re,sign*corr_master[5][0].im);
	}
...
#pragma omp parallel for private(lx,ly,lz,v,x,y,z,tmp)
	...
	corr[4][0] = (qcd_complex_16) {0,0};
	corr[5][0] = (qcd_complex_16) {0,0};
	corr[6][0] = (qcd_complex_16) {0,0};
	corr[7][0] = (qcd_complex_16) {0,0};
	corr[8][0] = (qcd_complex_16) {0,0};
	if(lt>=0 && lt<geo.lL[0])  //inside the local lattice, otherwise nothing to calculate
	  for(lx=0; lx<geo.lL[1]; lx++)
          for(ly=0; ly<geo.lL[2]; ly++)
	  for(lz=0; lz<geo.lL[3]; lz++)
	    ...
	    y=ly+geo.Pos[2]*geo.lL[2] - x_src[2];
	    z=lz+geo.Pos[3]*geo.lL[3] - x_src[3];
	    corr[0][0]=qcd_CADD(corr[0][0], qcd_CSCALE(block_n[mu][v],x));
	    corr[1][0]=qcd_CADD(corr[1][0], qcd_CSCALE(block_n[mu][v],y));
	    corr[2][0]=qcd_CADD(corr[2][0], qcd_CSCALE(block_n[mu][v],z));
	    corr[3][0]=qcd_CADD(corr[3][0], qcd_CSCALE(block_l[mu][v],x));
	    corr[4][0]=qcd_CADD(corr[4][0], qcd_CSCALE(block_l[mu][v],y));
	    corr[5][0]=qcd_CADD(corr[5][0], qcd_CSCALE(block_l[mu][v],z));
	    corr[6][0]=qcd_CADD(corr[6][0], qcd_CSCALE(block_a[mu][v],x));
	    corr[7][0]=qcd_CADD(corr[7][0], qcd_CSCALE(block_a[mu][v],y));
	    corr[8][0]=qcd_CADD(corr[8][0], qcd_CSCALE(block_a[mu][v],z));
	  }
	for(int j=0; j<9; j++)
	  MPI_Reduce(&(corr[j][0].re), &(corr_master[j][0].re), 2, MPI_DOUBLE, MPI_SUM, 0, MPI_COMM_WORLD);
	if(myid==0) 
	  {
	    fprintf(fp_corrnoe_xv,"%i %+i %+i %+i %+e %+e %i x\n",t,0,0,0,
		    sign*corr_master[0][0].re*0.25,sign*corr_master[0][0].im*0.25,mu);
	    fprintf(fp_corrnoe_xv,"%i %+i %+i %+i %+e %+e %i y\n",t,0,0,0,
		    sign*corr_master[1][0].re*0.25,sign*corr_master[1][0].im*0.25,mu);
	    fprintf(fp_corrnoe_xv,"%i %+i %+i %+i %+e %+e %i z\n",t,0,0,0,
		    sign*corr_master[2][0].re*0.25,sign*corr_master[2][0].im*0.25,mu);
	    fprintf(fp_corrloc_xv,"%i %+i %+i %+i %+e %+e %i x\n",t,0,0,0,
		    sign*corr_master[3][0].re,sign*corr_master[3][0].im,mu);
	    fprintf(fp_corrloc_xv,"%i %+i %+i %+i %+e %+e %i y\n",t,0,0,0,
		    sign*corr_master[4][0].re,sign*corr_master[4][0].im,mu);
	    fprintf(fp_corrloc_xv,"%i %+i %+i %+i %+e %+e %i z\n",t,0,0,0,
		    sign*corr_master[5][0].re,sign*corr_master[5][0].im,mu);
	    fprintf(fp_corrloc_xa,"%i %+i %+i %+i %+e %+e %i x\n",t,0,0,0,
		    sign*corr_master[6][0].re,sign*corr_master[6][0].im,mu);
	    fprintf(fp_corrloc_xa,"%i %+i %+i %+i %+e %+e %i y\n",t,0,0,0,
		    sign*corr_master[7][0].re,sign*corr_master[7][0].im,mu);
	    fprintf(fp_corrloc_xa,"%i %+i %+i %+i %+e %+e %i z\n",t,0,0,0,
		    sign*corr_master[8][0].re,sign*corr_master[8][0].im,mu);
	  }
      }//end mu loop
...
#pragma omp parallel for private(lx,ly,lz,v,x,y,z,tmp)
	...
	for(lx=0; lx<geo.lL[1]; lx++)
        ...
        for(lz=0; lz<geo.lL[3]; lz++)
	  {
	    ...
	    y=ly+geo.Pos[2]*geo.lL[2] - x_src[2];
	    z=lz+geo.Pos[3]*geo.lL[3] - x_src[3];
	    corr[0][0]=qcd_CADD(corr[0][0], qcd_CSCALE(block_vD[mu*4+nu][v],x));
	    corr[1][0]=qcd_CADD(corr[1][0], qcd_CSCALE(block_vD[mu*4+nu][v],y));
	    corr[2][0]=qcd_CADD(corr[2][0], qcd_CSCALE(block_vD[mu*4+nu][v],z));
	    corr[3][0]=qcd_CADD(corr[3][0], qcd_CSCALE(block_aD[mu*4+nu][v],x));
	    corr[4][0]=qcd_CADD(corr[4][0], qcd_CSCALE(block_aD[mu*4+nu][v],y));
	    corr[5][0]=qcd_CADD(corr[5][0], qcd_CSCALE(block_aD[mu*4+nu][v],z));
	    corr[6][0]=qcd_CADD(corr[6][0], qcd_CSCALE(block_d1[mu*4+nu][v],x));
	    corr[7][0]=qcd_CADD(corr[7][0], qcd_CSCALE(block_d1[mu*4+nu][v],y));
	    corr[8][0]=qcd_CADD(corr[8][0], qcd_CSCALE(block_d1[mu*4+nu][v],z));
	  }
	 for(int j=0; j<8; j++)   
	   MPI_Reduce(&(corr[j][0].re), &(corr_master[j][0].re), 2, MPI_DOUBLE, MPI_SUM, 0, MPI_COMM_WORLD);
	 if(myid==0) 
	   {
	     fprintf(fp_corr_xvD,"%i %+i %+i %+i %+e %+e %i %i x\n",t,0,0,0,
		     sign*corr_master[0][0].re*0.125,sign*corr_master[0][0].im*0.125,mu,nu);
	     fprintf(fp_corr_xvD,"%i %+i %+i %+i %+e %+e %i %i y\n",t,0,0,0,
		     sign*corr_master[1][0].re*0.125,sign*corr_master[1][0].im*0.125,mu,nu);
	     fprintf(fp_corr_xvD,"%i %+i %+i %+i %+e %+e %i %i z\n",t,0,0,0,
		     sign*corr_master[2][0].re*0.125,sign*corr_master[2][0].im*0.125,mu,nu);
	     fprintf(fp_corr_xaD,"%i %+i %+i %+i %+e %+e %i %i x\n",t,0,0,0,
		     sign*corr_master[3][0].re*0.125,sign*corr_master[3][0].im*0.125,mu,nu);
	     fprintf(fp_corr_xaD,"%i %+i %+i %+i %+e %+e %i %i y\n",t,0,0,0,
		     sign*corr_master[4][0].re*0.125,sign*corr_master[4][0].im*0.125,mu,nu);
	     fprintf(fp_corr_xaD,"%i %+i %+i %+i %+e %+e %i %i z\n",t,0,0,0,
		     sign*corr_master[5][0].re*0.125,sign*corr_master[5][0].im*0.125,mu,nu);
	   }   
	 if(myid==0)
	   if(mu != nu)
	     {
	       fprintf(fp_corr_xd1,"%i %+i %+i %+i %+e %+e %i %i\n",t,0,0,0,
		       sign*corr_master[6][0].re*0.125,sign*corr_master[6][0].im*0.125,mu,nu);
	       fprintf(fp_corr_xd1,"%i %+i %+i %+i %+e %+e %i %i\n",t,0,0,0,
		       sign*corr_master[7][0].re*0.125,sign*corr_master[7][0].im*0.125,mu,nu);
	       fprintf(fp_corr_xd1,"%i %+i %+i %+i %+e %+e %i %i\n",t,0,0,0,
		       sign*corr_master[8][0].re*0.125,sign*corr_master[8][0].im*0.125,mu,nu);
	     }
      }//end mu/nu loop    
      ...
      if(t==t_stop && myid==0)
      {
         ...
         fclose(fp_corrnoe_xv);
         fclose(fp_corrloc_xv);
         fclose(fp_corrloc_xs);
         fclose(fp_corrloc_xp);
         fclose(fp_corrloc_xa);   
         fclose(fp_corr_xvD);
         fclose(fp_corr_xaD);
         fclose(fp_corr_xd1);
