 ...
 * <nsources>5</nsources>
 * <n_tsink>3</n_tsink>
 * <t_sink_1>10</t_sink_1>
 * <projector_1>13</projector_1>
 * <seq_src_name_1>seq_src_Proj13_ts_10_list</seq_src_name_1>
 * <t_sink_2>10</t_sink_2>
 * <projector_2>3</projector_2>
 * <seq_src_name_2>seq_src_Proj3_ts_10_list</seq_src_name_2>
 * <t_sink_3>10</t_sink_3>
 * <projector_3>4</projector_3>
 * <seq_src_name_3>seq_src_Proj4_ts_10_list</seq_src_name_3>
 * <t_sink_4>12</t_sink_4>
 * <projector_4>3</projector_4>
 * <seq_src_name_4>seq_src_Proj3_ts_12_list</seq_src_name_4>
 * <t_sink_5>14</t_sink_5>
 * <projector_5>4</projector_5>
 * <seq_src_name_5>seq_src_Proj4_ts14_list</seq_src_name_5>
 *
 *
 *
 * Tomasz Korzec 2009
...
{  
   qcd_uint_4 i,k,mu,nu,ku,lu,c1,c2,v,x,y,z;   // loop variables
   qcd_uint_4 c3,c1p,c2p,c3p,ctr,ctr2;
   qcd_uint_4 cc1,cc2,a,b,gu,ju;
   qcd_uint_4 isource,tcheck,tuniq;                 // ..
   qcd_uint_4 *t_sink,*t_sink_uniq;
   int lt_sink;    // sink/source time-slice

   qcd_uint_2 n_tsink,nsources; // number of different tsinks and total number of sources
...
//   if(myid==0) printf("process %i: Got source time slice: %d\n",myid, t_src);
   ...
   if(myid==0) printf("process %i: Got total number of sources: %d\n",myid, nsources);
   
   source_name= malloc(nsources*sizeof(*source_name));
   source_type= malloc(nsources*sizeof(qcd_uint_2));
   t_sink= malloc(nsources*sizeof(qcd_uint_4));
   t_sink_uniq= malloc(nsources*sizeof(qcd_uint_4));
   
   for(i=0; i<nsources; i++){
     source_name[i]= malloc(qcd_MAX_STRING_LENGTH*sizeof(char));
     
     sprintf(tmp_string,"<tsink_%d>",i+1);
     sscanf(qcd_getParam(tmp_string,params,params_len),"%d",&t_sink[i]);
     if(myid==0) printf("Got #%d sink time slice: %d\n",i+1,t_sink[i]);
     if(t_sink[i] >= L[0])
       {
	 if(myid==0) fprintf(stderr, " Error: t_sink[%d] (=%d) >= L[0] (=%d),\n t_sink should be in [0, L[0]), did you forget to mod(t_sink, L[0]) ?\n",i, t_sink[i], L[0]);
	 exit(EXIT_FAILURE);
       }
   ...
   //Find unique sink times
   tuniq=0;
   t_sink_uniq[tuniq] = t_sink[0];
   tuniq++;
   for(i=1;i<nsources;i++){
     tcheck=0;
     for(k=0;k<tuniq;k++){
       if(t_sink[i] != t_sink_uniq[k]){
	 tcheck++;
       }
     }
     if(tcheck==tuniq){
       t_sink_uniq[tuniq] = t_sink[i];
       tuniq++;
     }
   }

   if(myid==0) printf("Got %d unique tsinks:\n",tuniq);
   for(i=0;i<tuniq;i++){
     if(myid==0) printf("tsink = %d\n",t_sink_uniq[i]);
   }
...
///////////////////////////////////////////////////////////////////////////////////////////////////
   ...
   for(k=0;k<tuniq;k++){
       ...
       for(c1=0;c1<3;c1++)
	 {
	   ...
	   for(i=0; i<nsmear; i++)
	     {
	       if(qcd_gaussIteration3d(&vec,&uAPE,alpha,t_sink_uniq[k]))
		 {
     ...
     if(myid==0) printf("propagators for tsink=%d smeared\n",t_sink_uniq[k]);
   }
   ...
   for(isource=0; isource<nsources; isource++)
   {
      ...
      if(myid==0) printf("creating source with Proj%i and t_sink=%d.\n",source_type[isource],t_sink[isource]);
      lt_sink = t_sink[isource] - geo.Pos[0]*geo.lL[0]; // local t_sink
      ...
      for(c1=0;c1<3;c1++)
      {
         ...
         for(i=0; i<nsmear; i++)
         {
            if(qcd_gaussIteration3d(&vec,&uAPE,alpha,t_sink[isource]))
            {
               fprintf(stderr,"process %i: Error while smearing!\n",geo.myid);
               exit(EXIT_FAILURE);
            }
	 ...
	 qcd_writeVector(tmp_name, qcd_PROP_LIME, mu, c1, &vec);
      ...
      if(myid==0) printf("seq. source %d smeared\n",isource);
