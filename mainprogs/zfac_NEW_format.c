...
int main(int argc,char* argv[])
{
   ...
   char vfun_n_name[qcd_MAX_STRING_LENGTH];     // output file name, noether current vertex function
   ...
   qcd_complex_16 vfun_n[4][4][4][3][3];        // vertex function noether current
   ...
   qcd_complex_16 vfun_t[4][4][4][3][3];        // vertex function local tensor current
   ...
   qcd_complex_16 vfun_tD[16][4][4][3][3];      // vertex function 1 derivative tensor operator
   qcd_complex_16 vfun_d1[16][4][4][3][3];      // vertex function 1 derivative d1 operator
   qcd_complex_16 vfun_tmp[16][4][4][3][3];     // temp variable
   ...
   strcpy(vfun_n_name,qcd_getParam("<vertex_function_noether_name>",params,params_len));
   ...
   if(myid==0) printf("Got momentum: [(%i+1)*2*pi/%i, %i*2*pi/%i, %i*2*pi/%i, %i*2*pi/%i] \n",pn[0],L[0],pn[1],L[1],pn[2],L[2],pn[3],L[3]);
   ...
   {
      j=0;
      if( (fp_vfun_n=fopen(vfun_n_name,"w"))==NULL) j++;
   ...
   memset(&(vfun_n[0][0][0][0][0].re),0,4*4*4*3*3*sizeof(qcd_complex_16));
   memset(&(vfun_v[0][0][0][0][0].re),0,4*4*4*3*3*sizeof(qcd_complex_16));
   memset(&(vfun_a[0][0][0][0][0].re),0,4*4*4*3*3*sizeof(qcd_complex_16));
   memset(&(vfun_t[0][0][0][0][0].re),0,4*4*4*3*3*sizeof(qcd_complex_16));
   ...
   memset(&(vfun_tD[0][0][0][0][0].re),0,16*4*4*3*3*sizeof(qcd_complex_16));
   ...
   {   
      ...
      { 
         ...
         for(ic3=0; ic3<3; ic3++)
         {
            if(qcd_NORM(qcd_ONE_PLUS_GAMMA[mu][id2][id3])>1e-4)
            {
                // x+mu  x  x
                vfun_n[mu][id1][id4][ic1][ic4] = qcd_CADD(vfun_n[mu][id1][id4][ic1][ic4],
                                                          qcd_CMUL(lprop.D[ip1][id1][id2][ic1][ic2],
                                                                   qcd_CMUL(qcd_ONE_PLUS_GAMMA[mu][id2][id3],
                                                                            qcd_CMUL(qcd_CONJ(u.D[i][mu][ic3][ic2]),
                                                                                     rprop.D[i][id3][id4][ic3][ic4]))));
                // x  x-mu  x-mu
                vfun_n[mu][id1][id4][ic1][ic4] = qcd_CADD(vfun_n[mu][id1][id4][ic1][ic4],
            ...
            if(qcd_NORM(qcd_ONE_MINUS_GAMMA[mu][id2][id3])>1e-4)
            {
                // x  x  x+mu
                vfun_n[mu][id1][id4][ic1][ic4] = qcd_CSUB(vfun_n[mu][id1][id4][ic1][ic4],
                                                          qcd_CMUL(lprop.D[i][id1][id2][ic1][ic2],
                                                                   qcd_CMUL(qcd_ONE_MINUS_GAMMA[mu][id2][id3],
                                                                            qcd_CMUL(u.D[i][mu][ic2][ic3],
                                                                                     rprop.D[ip1][id3][id4][ic3][ic4]))));
                // x-mu  x-mu  x
                vfun_n[mu][id1][id4][ic1][ic4] = qcd_CSUB(vfun_n[mu][id1][id4][ic1][ic4],
         ...
         for(mu=0; mu<4; mu++)
         if(qcd_NORM(g5sig[mu][0][id2][id3])>1e-4)
         {
            vfun_t[mu][id1][id4][ic1][ic4] = qcd_CADD(vfun_t[mu][id1][id4][ic1][ic4], qcd_CMUL(lxr,
                                                                                               g5sig[mu][0][id2][id3]));
         }
         ...
         {                       
            ...
            {        
               vfun_tD[mu*4+nu][id1][id4][ic1][ic4] = qcd_CADD(vfun_tD[mu*4+nu][id1][id4][ic1][ic4],
   ...
   MPI_Reduce(&(vfun_n[0][0][0][0][0].re), &(vfun_tmp[0][0][0][0][0].re), 4*4*4*3*3*2, MPI_DOUBLE, MPI_SUM, 0, MPI_COMM_WORLD);
   if(myid==0)   
   for(mu=0; mu<4; mu++)
   for(id1=0; id1<4; id1++)
   for(id4=0; id4<4; id4++)
   for(ic1=0; ic1<3; ic1++)
   for(ic4=0; ic4<3; ic4++)
      fprintf(fp_vfun_n,"%i %i %i %i %i %+e %+e\n",mu,id1,id4,ic1,ic4,vfun_tmp[mu][id1][id4][ic1][ic4].re,vfun_tmp[mu][id1][id4][ic1][ic4].im);
   ...
   MPI_Reduce(&(vfun_t[0][0][0][0][0].re), &(vfun_tmp[0][0][0][0][0].re), 4*4*4*3*3*2, MPI_DOUBLE, MPI_SUM, 0, MPI_COMM_WORLD);
   if(myid==0)   
   for(mu=0; mu<4; mu++)
   for(id1=0; id1<4; id1++)
   for(id4=0; id4<4; id4++)
   for(ic1=0; ic1<3; ic1++)
   for(ic4=0; ic4<3; ic4++)
      fprintf(fp_vfun_t,"%i %i %i %i %i %+e %+e\n",mu,id1,id4,ic1,ic4,vfun_tmp[mu][id1][id4][ic1][ic4].re,vfun_tmp[mu][id1][id4][ic1][ic4].im);
   ...
   for(ic1=0; ic1<3; ic1++)
   for(ic4=0; ic4<3; ic4++)
      fprintf(fp_vfun_vD,"%i %i %i %i %i %i %+e %+e\n",mu,nu,id1,id4,ic1,ic4,vfun_tmp[mu*4+nu][id1][id4][ic1][ic4].re,vfun_tmp[mu*4+nu][id1][id4][ic1][ic4].im);
   ...
   MPI_Reduce(&(vfun_tD[0][0][0][0][0].re), &(vfun_tmp[0][0][0][0][0].re), 16*4*4*3*3*2, MPI_DOUBLE, MPI_SUM, 0, MPI_COMM_WORLD);
   if(myid==0)
   for(mu=1; mu<4; mu++)
   for(nu=0; nu<4; nu++)
   for(id1=0; id1<4; id1++)
   ...
   for(ic1=0; ic1<3; ic1++)
   for(ic4=0; ic4<3; ic4++)
      fprintf(fp_vfun_tD,"%i %i %i %i %i %i %+e %+e\n",mu,nu,id1,id4,ic1,ic4,vfun_tmp[mu*4+nu][id1][id4][ic1][ic4].re,vfun_tmp[mu*4+nu][id1][id4][ic1][ic4].im);
