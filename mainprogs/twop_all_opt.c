...
int main(int argc,char* argv[])
{
   ...
   qcd_complex_16 **mcorrp[3],**mcorra[3],mcorrsum[6];
   qcd_real_8 plaq;
   qcd_int_4 ctr, ctr2;
   ...
   qcd_complex_16 *udblock,*dublock,*usblock,*sublock,*cdblock,*dcblock;          // for mesons
...
#pragma omp parallel
   ...
   for(mu=0;mu<4;mu++){
	...
	usblock = (qcd_complex_16*) malloc(geo.lV3*sizeof(qcd_complex_16));
	sublock = (qcd_complex_16*) malloc(geo.lV3*sizeof(qcd_complex_16));
	cdblock = (qcd_complex_16*) malloc(geo.lV3*sizeof(qcd_complex_16));
	dcblock = (qcd_complex_16*) malloc(geo.lV3*sizeof(qcd_complex_16));	
...
//------------------------------------------------------------MESONS------------------------------
    ...
    if(lt>=0 && lt<geo.lL[0]){  //inside the local lattice, otherwise nothing to calculate
		...
		for(v3=0; v3<geo.lV3; v3++){
			udblock[v3]= (qcd_complex_16) {0,0};   // set blocks to zero
			dublock[v3]= (qcd_complex_16) {0,0};   // set blocks to zero
			usblock[v3]= (qcd_complex_16) {0,0};   // set blocks to zero
			sublock[v3]= (qcd_complex_16) {0,0};   // set blocks to zero
			cdblock[v3]= (qcd_complex_16) {0,0};   // set blocks to zero
			dcblock[v3]= (qcd_complex_16) {0,0};   // set blocks to zero
		}

#pragma omp parallel for private(lz,ly,lx,v,mu,nu,c1,c2)
		for(v3=0; v3<lv3; v3++) {
			lz = v3 % geo.lL[3];
			...
			for(c2=0;c2<3;c2++){  
			  ...
			  //pion
			  udblock[v3] = qcd_CSUB( udblock[v3],qcd_CMUL(uprop_pb.D[v][mu][nu][c1][c2],qcd_CONJ(uprop_pb.D[v][mu][nu][c1][c2])) );
			  dublock[v3] = qcd_CSUB( dublock[v3],qcd_CMUL(dprop_pb.D[v][mu][nu][c1][c2],qcd_CONJ(dprop_pb.D[v][mu][nu][c1][c2])) );
			  //-kaon
			  usblock[v3] = qcd_CSUB( usblock[v3],qcd_CMUL(sprop_pb.D[v][mu][nu][c1][c2],qcd_CONJ(uprop_pb.D[v][mu][nu][c1][c2])) );
			  sublock[v3] = qcd_CSUB( sublock[v3],qcd_CMUL(uprop_pb.D[v][mu][nu][c1][c2],qcd_CONJ(sprop_pb.D[v][mu][nu][c1][c2])) );
			  //-Dmeson
			  cdblock[v3] = qcd_CSUB( cdblock[v3],qcd_CMUL(dprop_pb.D[v][mu][nu][c1][c2],qcd_CONJ(cprop_pb.D[v][mu][nu][c1][c2])) );
			  dcblock[v3] = qcd_CSUB( dcblock[v3],qcd_CMUL(cprop_pb.D[v][mu][nu][c1][c2],qcd_CONJ(dprop_pb.D[v][mu][nu][c1][c2])) );
...
//Fourier transform time-slice
        ...
        for(j=0; j<nmom; j++){
			...
			for(i=0;i<6;i++) mcorrsum[i] = (qcd_complex_16) {0,0};
            ...
            for(lz=0; lz<geo.lL[3]; lz++){  
				...
				mcorrsum[0] = qcd_CADD(mcorrsum[0], qcd_CMUL(udblock[v3],C2));
				mcorrsum[1] = qcd_CADD(mcorrsum[1], qcd_CMUL(dublock[v3],C2));
				mcorrsum[2] = qcd_CADD(mcorrsum[2], qcd_CMUL(usblock[v3],C2));
				mcorrsum[3] = qcd_CADD(mcorrsum[3], qcd_CMUL(sublock[v3],C2));
				mcorrsum[4] = qcd_CADD(mcorrsum[4], qcd_CMUL(cdblock[v3],C2));
				mcorrsum[5] = qcd_CADD(mcorrsum[5], qcd_CMUL(dcblock[v3],C2));                                             
            }
