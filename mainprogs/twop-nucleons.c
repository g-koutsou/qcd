...
main(int argc,char* argv[])
{
  ...
  char corr_p_name[n_two_points][MAX_STRING];
  for(int i=0; i<n_two_points; i++) {
    ...
    sprintf(tag,"<corr_name_%d>",i);
    strcpy(corr_p_name[i],qcd_getParam(tag,params,params_len));
    if(myid==0) 
...
#pragma omp parallel for
    ...
    if(myid==0) {
      FILE *fp = fopen(corr_p_name[itwop], "w");
      if(fp == NULL) {
