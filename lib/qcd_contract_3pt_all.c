...
int qcd_projector_3pt(qcd_complex_16 *block[5], qcd_complex_16 *block_pr[10][4][4], qcd_geometry *geo, qcd_uint_4 prid){
  ...
  for(lz=0; lz<geo->lL[3]; lz++){
	...
	for(ga =0; ga <4; ga ++) 
	for(gap=0; gap<4; gap++)
		if(qcd_NORM(PROJECTOR[proj][ga][gap])>0.0001){
    
  			block[prid][v3] = qcd_CADD( block[prid][v3],qcd_CMUL(PROJECTOR[proj][gap][ga],block_pr[9][ga][gap][v3]) );
								
		}//-non vanishing projector condition	
  }//-space
...
int qcd_projector32_3pt(qcd_complex_16 gamma12[4][4], qcd_complex_16 gamma13[4][4], qcd_complex_16 gamma23[4][4], 
  ...
  for(lz=0; lz<geo->lL[3]; lz++){
	...
	for(gap=0; gap<4; gap++){
		...
		multiterm[ga][gap] = (qcd_complex_16) {0,0};
		
		for(j=0;j<4;j++){
			multiterm[ga][gap] = qcd_CADD(multiterm[ga][gap],
								 qcd_CADD(qcd_CMUL(gamma12[ga][j],qcd_CSUB(block_pr[3][j][gap][v3],block_pr[1][j][gap][v3])),
								 qcd_CADD(qcd_CMUL(gamma13[ga][j],qcd_CSUB(block_pr[6][j][gap][v3],block_pr[2][j][gap][v3])),
										  qcd_CMUL(gamma23[ga][j],qcd_CSUB(block_pr[7][j][gap][v3],block_pr[5][j][gap][v3])))));
		}//-multiterm j loop

		multiterm[ga][gap] = qcd_CSCALE(multiterm[ga][gap],1.0/6.0); 
		diagterm[ga][gap] = qcd_CSCALE(qcd_CADD(block_pr[0][ga][gap][v3],qcd_CADD(block_pr[4][ga][gap][v3],block_pr[8][ga][gap][v3])),1.0/3.0);
 
		block_pr[9][ga][gap][v3] = qcd_CSUB(diagterm[ga][gap],multiterm[ga][gap]);  
 
  
/*		block[prid][v3] = qcd_CADD(block[prid][v3],qcd_CMUL(PROJECTOR[proj][gap][ga],
					                                    qcd_CSUB(diagterm[ga][gap],multiterm[ga][gap]) 
						                                   )
							      );
*/								
		}//-ga, gap
...
int qcd_lambdas_f1_3pt(qcd_int_4 ctr, qcd_int_2 cgcg_ind[16*16][4],qcd_complex_16 cgcg_val[16*16], qcd_complex_16 *block[10][4][4],
		                qcd_propagator *propf2, qcd_propagator *propf3, qcd_propagator *seqpropf1, qcd_geometry *geo, qcd_uint_4 lt, qcd_uint_4 elem){
...
//-- Allocation of local blocks  
	...
	if( particles32[p_id] ){
		...
		for(j=0;j<4;j++)
		for(k=0;k<4;k++){	
			block_pr[9][j][k] = (qcd_complex_16*) malloc(geo->lV3*sizeof(qcd_complex_16));
			
			if(block_pr[9][j][k]==NULL){
				printf("Block 9 %d %d not properly initialized\n",j,k);
				exit(EXIT_FAILURE);
			}

			for(v3=0;v3<(geo->lV3);v3++){
				block_pr[9][j][k][v3] = (qcd_complex_16) {0,0};
			}
		}				
	}
...
//-----------------------------------------------  
  ...
  switch(p_id){
	case 1://-proton
		switch(np){
			case 0:
				check += qcd_f1f2f1_f1_3pt(ctr5, cg5cg5_ind, cg5cg5_val, block_pr, uprop, dprop, sequprop, geo, lt,9);				
			break;
			case 1:
				check += qcd_f1f2f1_f2_3pt(ctr5, cg5cg5_ind, cg5cg5_val, block_pr, uprop, dprop, seqdprop, geo, lt,9);
			break;
		}	 	  
	break;
	...
	case 2://-neutron
		switch(np){
			case 0:
				check += qcd_f1f2f1_f2_3pt(ctr5, cg5cg5_ind, cg5cg5_val, block_pr, dprop, uprop, sequprop, geo, lt,9);
			break;
			case 1:
				check += qcd_f1f2f1_f1_3pt(ctr5, cg5cg5_ind, cg5cg5_val, block_pr, dprop, uprop, seqdprop, geo, lt,9);
			break;
		}	 	  
	break;
	...
	case 3://-lambda
		switch(np){
			case 0:
				check += qcd_lambdas_f1_3pt(ctr5, cg5cg5_ind, cg5cg5_val, block_pr, dprop, sprop, sequprop, geo, lt,9);
			break;
			case 1:
				check += qcd_lambdas_f2_3pt(ctr5, cg5cg5_ind, cg5cg5_val, block_pr, uprop, sprop, seqdprop, geo, lt,9);
			break;
			case 2:
				check += qcd_lambdas_f3_3pt(ctr5, cg5cg5_ind, cg5cg5_val, block_pr, uprop, dprop, seqsprop, geo, lt,9);
			break;			
		}	 	  
	break;		
	...
	case 4://-sigma plus
		switch(np){
			case 0:
				check += qcd_f1f2f1_f1_3pt(ctr5, cg5cg5_ind, cg5cg5_val, block_pr, uprop, sprop, sequprop, geo, lt,9);	
			break;
			case 2:
				check += qcd_f1f2f1_f2_3pt(ctr5, cg5cg5_ind, cg5cg5_val, block_pr, uprop, sprop, seqsprop, geo, lt,9);	
			break;
		}	 	  
	break;
	...
	case 5://-sigma zero
		switch(np){
			case 0:
				check += qcd_f123f321_f1_3pt(ctr5, cg5cg5_ind, cg5cg5_val, block_pr, sprop, dprop, sequprop, geo, lt,9);	
			break;
			case 1:
				check += qcd_f123f321_f3_3pt(ctr5, cg5cg5_ind, cg5cg5_val, block_pr, uprop, sprop, seqdprop, geo, lt,9);	
			break;
			case 2:
				check += qcd_f123f321_f2_3pt(ctr5, cg5cg5_ind, cg5cg5_val, block_pr, uprop, dprop, seqsprop, geo, lt,9);	
			break;			
		}	 	  
	break;	
	...
	case 6://-sigma minus
		switch(np){
			case 1:
				check += qcd_f1f2f1_f1_3pt(ctr5, cg5cg5_ind, cg5cg5_val, block_pr, dprop, sprop, seqdprop, geo, lt,9);
			break;
			case 2:
				check += qcd_f1f2f1_f2_3pt(ctr5, cg5cg5_ind, cg5cg5_val, block_pr, dprop, sprop, seqsprop, geo, lt,9);	
			break;
		}	 	  
	break;
	...
	case 7://-xi zero
		switch(np){
			case 0:
				check += qcd_f1f2f1_f2_3pt(ctr5, cg5cg5_ind, cg5cg5_val, block_pr, sprop, uprop, sequprop, geo, lt,9);	
			break;
			case 2:
				check += qcd_f1f2f1_f1_3pt(ctr5, cg5cg5_ind, cg5cg5_val, block_pr, sprop, uprop, seqsprop, geo, lt,9);	
			break;
		}	 	  
	break;
	...
	case 8://-xi minus
		switch(np){
			case 1:
				check += qcd_f1f2f1_f2_3pt(ctr5, cg5cg5_ind, cg5cg5_val, block_pr, sprop, dprop, seqdprop, geo, lt,9);	
			break;
			case 2:
				check += qcd_f1f2f1_f1_3pt(ctr5, cg5cg5_ind, cg5cg5_val, block_pr, sprop, dprop, seqsprop, geo, lt,9);	
			break;
		} 	  
	break;			
//-----------------------------------------------------------------------------------	
	case 9://-delta plus plus
	  ...
	  check += qcd_projector32_3pt(gamma12,gamma13,gamma23,block_pr,geo);    	  
	break;
...
//--------------------------------------------------------------------------------	
	case 19://-lambda c plus
		switch(np){
			case 0:
				check += qcd_lambdas_f1_3pt(ctr5, cg5cg5_ind, cg5cg5_val, block_pr, dprop, cprop, sequprop, geo, lt,9);	
			break;
			case 1:
				check += qcd_lambdas_f2_3pt(ctr5, cg5cg5_ind, cg5cg5_val, block_pr, uprop, cprop, seqdprop, geo, lt,9);	
			break;
			case 3:
				check += qcd_lambdas_f3_3pt(ctr5, cg5cg5_ind, cg5cg5_val, block_pr, uprop, dprop, seqcprop, geo, lt,9);	
			break;			
		}	 	  
	break;
	...
	case 20://-sigma c plus plus
		switch(np){
			case 0:
				check += qcd_f1f2f1_f1_3pt(ctr5, cg5cg5_ind, cg5cg5_val, block_pr, uprop, cprop, sequprop, geo, lt,9);	
			break;
			case 3:
				check += qcd_f1f2f1_f2_3pt(ctr5, cg5cg5_ind, cg5cg5_val, block_pr, uprop, cprop, seqcprop, geo, lt,9);	
			break;
		}	 	  
	break;
	...
	case 21://-sigma c plus
		switch(np){
			case 0:
				check += qcd_f123f321_f1_3pt(ctr5, cg5cg5_ind, cg5cg5_val, block_pr, cprop, dprop, sequprop, geo, lt,9);	
			break;
			case 1:
				check += qcd_f123f321_f3_3pt(ctr5, cg5cg5_ind, cg5cg5_val, block_pr, uprop, cprop, seqdprop, geo, lt,9);
			break;	
			case 3:
				check += qcd_f123f321_f2_3pt(ctr5, cg5cg5_ind, cg5cg5_val, block_pr, uprop, dprop, seqcprop, geo, lt,9);					
			break;
		}	 	  
	break;	
	...
	case 22://-sigma c zero
		switch(np){
			case 1:
				check += qcd_f1f2f1_f1_3pt(ctr5, cg5cg5_ind, cg5cg5_val, block_pr, dprop, cprop, seqdprop, geo, lt,9);
			break;
			case 3:
				check += qcd_f1f2f1_f2_3pt(ctr5, cg5cg5_ind, cg5cg5_val, block_pr, dprop, cprop, seqcprop, geo, lt,9);	
			break;
		}	 	  
	break;
	...
	case 23://-xi c plus
		switch(np){
			case 0:
				check += qcd_f1f2f3_3pt(ctr5, cg5cg5_ind, cg5cg5_val, block_pr, sequprop, sprop, cprop, geo, lt,9);	
			break;			
			case 2:
				check += qcd_f1f2f3_3pt(ctr5, cg5cg5_ind, cg5cg5_val, block_pr, uprop, seqsprop, cprop, geo, lt,9);	
			break;
			case 3:
				check += qcd_f1f2f3_3pt(ctr5, cg5cg5_ind, cg5cg5_val, block_pr, uprop, sprop, seqcprop, geo, lt,9);	
			break;
		}	 	  
	break;
	...
	case 24://-xi prime c plus
		switch(np){
			case 0:
				check += qcd_f123f321_f1_3pt(ctr5, cg5cg5_ind, cg5cg5_val, block_pr, cprop, sprop, sequprop, geo, lt,9);	
			break;
			case 2:
				check += qcd_f123f321_f3_3pt(ctr5, cg5cg5_ind, cg5cg5_val, block_pr, uprop, cprop, seqsprop, geo, lt,9);
			break;
		    case 3:
				check += qcd_f123f321_f2_3pt(ctr5, cg5cg5_ind, cg5cg5_val, block_pr, uprop, sprop, seqcprop, geo, lt,9);					
			break;
		}	 	  
	break;	
	...
	case 25://-xi c zero
		switch(np){
			case 1:
				check += qcd_f1f2f3_3pt(ctr5, cg5cg5_ind, cg5cg5_val, block_pr, seqdprop, sprop, cprop, geo, lt,9);	
			break;			
			case 2:
				check += qcd_f1f2f3_3pt(ctr5, cg5cg5_ind, cg5cg5_val, block_pr, dprop, seqsprop, cprop, geo, lt,9);	
			break;
			case 3:
				check += qcd_f1f2f3_3pt(ctr5, cg5cg5_ind, cg5cg5_val, block_pr, dprop, sprop, seqcprop, geo, lt,9);	
			break;
		}	 	  
	break;
	...
	case 26://-xi prime c zero
		switch(np){
			case 1:
				check += qcd_f123f321_f1_3pt(ctr5, cg5cg5_ind, cg5cg5_val, block_pr, cprop, sprop, seqdprop, geo, lt,9);	
			break;
			case 2:
				check += qcd_f123f321_f3_3pt(ctr5, cg5cg5_ind, cg5cg5_val, block_pr, dprop, cprop, seqsprop, geo, lt,9);
			break;	
			case 3:
				check += qcd_f123f321_f2_3pt(ctr5, cg5cg5_ind, cg5cg5_val, block_pr, dprop, sprop, seqcprop, geo, lt,9);					
			break;
		}	 	  
	break;
	...
	case 27://-omega c zero
		switch(np){
			case 2:
				check += qcd_f1f2f1_f1_3pt(ctr5, cg5cg5_ind, cg5cg5_val, block_pr, sprop, cprop, seqsprop, geo, lt,9);	
			break;
			case 3:
				check += qcd_f1f2f1_f2_3pt(ctr5, cg5cg5_ind, cg5cg5_val, block_pr, sprop, cprop, seqcprop, geo, lt,9);	
			break;
		}	 	  
	break;	
//--------------------------------------------------------------------------------	
...
//----------------------------------------------------------------------------------- 	
	case 34://-xi c c plus plus
		switch(np){
			case 0:
				check += qcd_f1f2f1_f2_3pt(ctr5, cg5cg5_ind, cg5cg5_val, block_pr, cprop, uprop, sequprop, geo, lt,9);	
			break;
			case 3:
				check += qcd_f1f2f1_f1_3pt(ctr5, cg5cg5_ind, cg5cg5_val, block_pr, cprop, uprop, seqcprop, geo, lt,9);	
			break;
		}	 	  
	break;
	...
	case 35://-xi c c plus
		switch(np){
			case 1:
				check += qcd_f1f2f1_f2_3pt(ctr5, cg5cg5_ind, cg5cg5_val, block_pr, cprop, dprop, seqdprop, geo, lt,9);	
			break;
			case 3:
				check += qcd_f1f2f1_f1_3pt(ctr5, cg5cg5_ind, cg5cg5_val, block_pr, cprop, dprop, seqcprop, geo, lt,9);	
			break;
		} 	  
	break;
	...
	case 36://-omega c c plus
		switch(np){
			case 2:
				check += qcd_f1f2f1_f2_3pt(ctr5, cg5cg5_ind, cg5cg5_val, block_pr, cprop, sprop, seqsprop, geo, lt,9);	
			break;
			case 3:
				check += qcd_f1f2f1_f1_3pt(ctr5, cg5cg5_ind, cg5cg5_val, block_pr, cprop, sprop, seqcprop, geo, lt,9);	
			break;
		} 	  
	break;				
//-----------------------------------------------------------------------------------       
...
//-----------------------------------------------------------------------------------	
	case 41://-proton extra
		switch(np){
			case 0:
				check += qcd_pnextra_f1_3pt(ctr, cgcg_ind, cgcg_val, block_pr, uprop, dprop, sequprop, geo, lt,9);				
			break;
			case 1:
				check += qcd_pnextra_f2_3pt(ctr, cgcg_ind, cgcg_val, block_pr, uprop, dprop, seqdprop, geo, lt,9);
			break;
		}	 	  
	break;
	...
	case 42://-neutron extra
		switch(np){
			case 0:
				check += qcd_pnextra_f2_3pt(ctr, cgcg_ind, cgcg_val, block_pr, dprop, uprop, sequprop, geo, lt,9);
			break;
			case 1:
				check += qcd_pnextra_f1_3pt(ctr, cgcg_ind, cgcg_val, block_pr, dprop, uprop, seqdprop, geo, lt,9);
			break;
		}	 	  
	break;
//-----------------------------------------------------------------------------------	
...
//-----------------------------------------------------------------------------------
	case 45://-xi_cplus extra
		switch(np){
			case 0:
				check += qcd_lambdas_f2_3pt(ctr5, cg5cg5_ind, cg5cg5_val, block_pr, sprop, cprop, sequprop, geo, lt,9);
			break;
			case 2:
				check += qcd_lambdas_f1_3pt(ctr5, cg5cg5_ind, cg5cg5_val, block_pr, uprop, cprop, seqsprop, geo, lt,9);
			break;
			case 3:
				check += qcd_lambdas_f3_3pt(ctr5, cg5cg5_ind, cg5cg5_val, block_pr, sprop, uprop, seqcprop, geo, lt,9);
			break;			
		}	 	  
	break;
	...
	case 46://-xi_czero extra
		switch(np){
			case 1:
				check += qcd_lambdas_f2_3pt(ctr5, cg5cg5_ind, cg5cg5_val, block_pr, sprop, cprop, seqdprop, geo, lt,9);
			break;
			case 2:
				check += qcd_lambdas_f1_3pt(ctr5, cg5cg5_ind, cg5cg5_val, block_pr, dprop, cprop, seqsprop, geo, lt,9);
			break;
			case 3:
				check += qcd_lambdas_f3_3pt(ctr5, cg5cg5_ind, cg5cg5_val, block_pr, sprop, dprop, seqcprop, geo, lt,9);
			break;			
		}	 	  
	break;
//-----------------------------------------------------------------------------------
...
//-----------------------------------------------------------------------------------	   
	...
	for(k=0;k<5;k++) check += qcd_projector_3pt(block,block_pr,geo,k);	//--project the blocks
	
	if(geo->myid==0){
		if(particles32[p_id]){
			if ( check==15 ) printf("ROUTINE %s part %s t=%d\n",particle_names[p_id],particles_parts[p_id][np], lt);
		}
		else{
			if ( check==6 )  printf("ROUTINE %s part %s t=%d\n",particle_names[p_id],particles_parts[p_id][np], lt);
		}
	}	
